...
interface Exercise {
  ...
  description: string;
  duration: string;
  category: 'breathing' | 'mindfulness' | 'grounding';
...
const ExercisesPage: React.FC = () => {
  ...
  const categoryColors = {
    breathing: 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800',
    mindfulness: 'bg-purple-50 dark:bg-purple-900/20 border-purple-200 dark:border-purple-800',
    grounding: 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800'
  };
  ...
  const startExercise = (exerciseId: string) => {
    if (exerciseId === 'breathing-4-4-6' || exerciseId === 'box-breathing') {
    ...
    // Set different durations based on exercise type
    if (exerciseId === 'grounding-5-4-3-2-1') {
      setMaxStepTime(30);
    } else if (exerciseId === 'body-scan') {
      setMaxStepTime(60);
    } else {
      setMaxStepTime(45);
    }
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {selectedExercise ? (
          ...
          <div className="max-w-3xl mx-auto">
            <div className="bg-white dark:bg-[#2C2542] rounded-2xl shadow-xl p-8 border border-gray-200 dark:border-gray-600">
              {(() => {
                const exercise = exercises.find(e => e.id === selectedExercise);
                ...
                return (
                  <>
                    <div className="text-center mb-8">
                      <div className="flex justify-center mb-4">{exercise.icon}</div>
                      <h2 className="text-3xl font-bold text-[#2E2E2E] dark:text-[#EDE9FE] mb-3">
                        {exercise.title}
                      </h2>
                      <p className="text-gray-600 dark:text-gray-300 text-lg">{exercise.description}</p>
                    </div>
                      ...
                      <div className="flex items-center justify-between mb-6">
                        ...
                        <div className="flex items-center space-x-3">
                          <div className="w-32 bg-gray-200 dark:bg-gray-600 rounded-full h-3">
                            <div 
                              className="bg-[#A78BFA] dark:bg-[#C4B5FD] h-3 rounded-full transition-all duration-300"
                              style={{ width: `${((currentStep + 1) / exercise.steps.length) * 100}%` }}
                            />
                          </div>
                          {isPlaying && (
                            <div className="text-sm text-gray-500 dark:text-gray-400 font-mono">
                              {maxStepTime - stepTimer}s
                            </div>
                          )}
                        </div>
                      </div>
                      
                      <div className="bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-700 dark:to-gray-800 rounded-xl p-8 min-h-[140px] flex items-center justify-center border border-gray-200 dark:border-gray-600">
                        <p className="text-xl text-center text-[#2E2E2E] dark:text-[#EDE9FE] leading-relaxed font-medium">
                    ...
                    <div className="flex flex-col sm:flex-row justify-center items-center space-y-4 sm:space-y-0 sm:space-x-4">
                      <div className="flex space-x-3">
                        <button
                          ...
                          className="flex items-center space-x-2 bg-gray-500 hover:bg-gray-600 disabled:bg-gray-300 disabled:cursor-not-allowed text-white px-4 py-3 rounded-xl font-medium transition-colors"
                        >
                          <ArrowLeft size={18} />
                          <span>Previous</span>
                        </button>
                        ...
                        <button
                          onClick={isPlaying ? pauseExercise : playExercise}
                          className="flex items-center space-x-2 bg-[#A78BFA] hover:bg-[#9333EA] dark:bg-[#C4B5FD] dark:hover:bg-[#A78BFA] text-white dark:text-[#1E1B2E] px-6 py-3 rounded-xl font-medium transition-colors shadow-lg"
                        >
                      ...
                      <div className="flex space-x-3">
                        <button
                          ...
                          className="flex items-center space-x-2 bg-gray-500 hover:bg-gray-600 text-white px-4 py-3 rounded-xl font-medium transition-colors"
                        >
                          <RotateCcw size={18} />
                          <span>Reset</span>
                        </button>
                        ...
                        <button
                          ...
                          className="px-6 py-3 border-2 border-gray-300 dark:border-gray-600 text-[#2E2E2E] dark:text-[#EDE9FE] rounded-xl font-medium hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
                        >
                          Back to Exercises
                        </button>
                      </div>
